...
const runMainApplication = () => {
    const { auth, db } = window._fb;
        ...
        const renderTable = () => {
            ...
            } else if (state.activeViewType === 'pec') {
                showView(pecWrapper, () => {
        ...
        const init = async () => {
            try {
                ...
                if (sidebarAiAssist) {
                    sidebarAiAssist.onclick = (e) => {
                        e.preventDefault();
                        state.activeViewType = 'ai_assist';
                        renderSidebar(); renderTable();
                    };
                }
                ...
                // ── Daily Wage Ledger nav handler ───────────────────────
                const sidebarWagesDaily = document.getElementById('sidebar-wages-daily');
                if (sidebarWagesDaily) {
                    sidebarWagesDaily.onclick = (e) => {
                        e.preventDefault();
                ...
                // ── PEC Application nav handler ─────────────────────────
                const sidebarPec = document.getElementById('sidebar-pec');
                if (sidebarPec) {
                    sidebarPec.onclick = (e) => {
                        e.preventDefault();
                        if (!state.pec) state.pec = {};
                        state.activeViewType = 'pec';
                        renderSidebar(); renderTable();
                    };
                }
                ...
                // Load Daily Wage Ledger data (job card summary import — shared across all users)
                window._wagesDailyDb = db;
                try {
                    if (cloudUnreachable) throw new Error('cloud unreachable — skipped');
                    const wdSnap = await readWithTimeout('shared/wages_daily_data');
                    const wdData = wdSnap.val();
                    if (wdData) {
                        state.wagesDaily = JSON.parse(wdData);
                        console.log("Daily wage ledger data loaded from cloud.");
                    } else if (!state.wagesDaily) {
                        state.wagesDaily = {};
                    }
                    window._sharedLoadOk['shared/wages_daily_data'] = true;
                } catch (e) {
                ...
                // Load PEC Application data (Forest Dept applications register — shared across all
                // users). Same guard as Tree Logs: window._pecLoaded gates savePecData so a failed
                // load can never be persisted over the real cloud data.
                window._pecDb = db;
                window._pecLoaded = false;
                for (let attempt = 1; attempt <= 4 && !cloudUnreachable; attempt++) {
                    try {
                        const pecSnap = await readWithTimeout('shared/pec_data');
                        const pecData = pecSnap.val();
                        if (pecData) {
                            state.pec = JSON.parse(pecData);
                            console.log("PEC application data loaded from cloud.");
                        } else if (!state.pec) {
                            state.pec = {};
                        }
                        window._pecLoaded = true;
                        window._sharedLoadOk['shared/pec_data'] = true;
                        break;
                    } catch (e) {
