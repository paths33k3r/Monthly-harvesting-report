...
```html
render_ironhorse.js?v=5
render_spraying.js?v=11
render_manuring.js?v=11
render_reports.js?v=21
script.js?v=29
```
When editing a file, increment its `?v=N` in `index.html`.

## Current branch
`2026-05-04_08-16-10` (branched from main for Iron Horse feature work)
...
### Pending / unresolved
- **Iron Horse template download**: User reported downloaded file "not in Excel format" (UUID filename in Chrome, 7.3 KB).  
  **Root cause identified**: `a.click()` without appending anchor to DOM causes browser to ignore `download` attribute; `URL.revokeObjectURL` called immediately before download starts.  
  **Fix needed** (not yet applied): append anchor to `document.body`, click, then `setTimeout(() => { URL.revokeObjectURL(url); a.remove(); }, 1000)`. Also replaced `{ formula: ... }` cells with static `0` to avoid ExcelJS XML validity issues (this part was partially applied — the Total cell formula was replaced but Grand Total formulas may still use formula syntax).  
  **File**: `render_ironhorse.js` → `downloadIronHorseTemplate()` around line 827–834.

- **Iron Horse Expenses import**: Wired up but not fully tested with real user data yet.

### Can be deleted
- `Spraying_GLY_ALLY_2026.xlsx` in project root — generated by Node.js for testing only.
