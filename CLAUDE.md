...
- **Scan** (`lvScanForm`): photo or PDF (pdf.js first-page rasterise, same lazy loader pattern as render_wages_daily) → downscaled JPEG stored as the `form` attachment on Save → **Google Vision** `DOCUMENT_TEXT_DETECTION` (OAuth scope `cloud-vision`, `languageHints:['en']` only — hinting `zh` too makes Vision misread Latin handwriting as CJK) → `lvParseFormOcr` prefills the editor. Review-first: amber notes, never saves directly. OCR failure still opens a blank editor with the photo attached on Save. **Requires the Cloud Vision API + billing enabled** (first 1,000 scans/month free). Raw response kept at **`window._lvLastOcr`** (`{text, fta, parsed}`) — handwriting OCR needs tuning against real scans and the parsed result alone doesn't show *why* a field failed.
- **OCR extraction is geometry-based, and the geometry is subtle** (`lvVisionWords` → `lvGroupLines` → `lvFindLabel` → `lvValueWordsFor`). Hard-won specifics:
  - **Rows locate labels; they must NOT collect values.** Printed labels share a baseline and group cleanly, but handwriting is ~2× taller and rides high, so it lands outside the label's row band — the first attempt lost whole fields and kept stray fragments (remarks came back as `"th"`, a piece of "Church"). `lvValueWordsFor` therefore searches **all** words to the label's right within a generous vertical band (`max(labelHeight*1.3, 16)`), not the label's row.
  - **That band spans neighbouring rows** — which is how the printed Chinese type labels (年假/病假/事假), sitting one row above and inside the English brackets' x-range, registered as ticks on every option. Tick candidates must be non-CJK and tick-shaped; a loose "any 1–2 char word inside the brackets" rule silently picks the wrong leave type.
  - **Anchor on the FULL compound label** (`DURING LEAVE Address`, not `DURING LEAVE`) or the trailing label word leaks into the value.
  - **Dates**: prefer the DATE OF LEAVE band + its wrapped continuation row, but **fall back to scanning the whole document** — the header `Date :` is the only other date on the form and is excluded, so anything else found is a leave date. Separators are matched loosely (`/ | . , ) \ l -`: handwritten slashes transcribe inconsistently) but the shape is validated (day 1-31, month 1-12), and the form's own `( N ) DAYS` count is cross-checked into a note.
  - Applicant is **fuzzy-matched to the Employee Master** (token overlap, 1-edit tolerance — validated: "BABOH AE LONDEK" → "BABOH AK LONDEK"); `lvTidy` strips punctuation the scanner picks off the ruled lines; `lvStripCJK` is the last-resort backstop on every text field.
